...
class TelegramBotRunner:
    ...
    async def handle_text(self, chat_id: int, text: str):
        ...
        # 2. أوامر تحميل ملف الـ PDF المباشرة وزر لوحة المفاتيح
        if text_clean in [
            "/pdf", "/tax_pdf", "pdf", "PDF", "بي دي اف",
            "📄 تحميل إقرار PDF", "تحميل إقرار PDF", "تحميل pdf", "تقرير pdf",
            "ملف pdf", "تنزيل pdf", "طباعة pdf", "تقرير الضريبة pdf", "تحميل التقرير pdf"
        ]:
            await self._send_tax_report_pdf(chat_id, all_time=True, label="كافة العمليات المسجلة")
            return
